...
const Manager = () => {
  ...
  const validateForm = () => {
    const { username, password, url } = form;
    const newErrors = {};
    ...
    if (!url || url.length < 3) {
      newErrors.url = "URL must be at least 3 characters long";
    }
  ...
  return (
    <main className='min-w-full min-h-[700px] mt-[70px] bg-gray-100'>
      <ToastContainer />
      ...
      <div className="passList flex justify-center mt-10">
        <table className="w-[90%] bg-white shadow-md rounded-lg overflow-hidden">
          ...
          <tbody className="tbody text-center">
            {passwordArray.map((obj) => (
              <tr key={obj.id} className="bg-gray-100 odd:bg-white hover:bg-gray-200 transition">
                <td className="py-3 flex justify-start items-center">
                  <span>{maskText(obj.url)}</span>
                  <span onClick={() => copyText(obj.url)} className="cursor-pointer ml-2">
                    <lord-icon
                ...
                <td className="py-3">
                  ...
                  <span onClick={() => copyText(obj.username)} className="cursor-pointer ml-2">
                    <lord-icon
                      src="https://cdn.lordicon.com/iykgtsbt.json"
                      trigger="hover"
                      style={{ width: '25px', height: '25px' }}
                ...
                <td className="py-3 flex text-left items-center justify-start gap-2">
                  ...
                  <span onClick={() => copyText(obj.password)} className="cursor-pointer ml-2">
                    <lord-icon
                      src="https://cdn.lordicon.com/iykgtsbt.json"
                      trigger="hover"
                      style={{ width: '25px', height: '25px' }}
                ...
                <td>
                  <span onClick={() => editPassword(obj.id)} className="cursor-pointer ml-2">
                    <lord-icon
                  ...
                  <span onClick={() => deletePassword(obj.id)} className="cursor-pointer ml-2">
                    <lord-icon
